...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        # }) s
        print("cur_stats_update")
        print(cur_stats)
        cur_stats.update({
            k: sum(d.get(k, 0) if isinstance(d.get(k,0), (int, float)) else 0 for d in infos) 
            for k in set.union(*[set(d) for d in infos])
        }) 
        print(cur_stats)
        # cur_stats.update({
        #     k: sum(int(d.get(k, 0)) if isinstance(d.get(k, 0), (int, float)) else 0
        #         for d in infos)
        #     for k in set.union(*[set(d) for d in infos])
        # })
        # print("!!cur_stats!! DEBUG")
        # print(cur_stats)
        # cur_stats.update({
        #     k: sum(
        #         (sum(d.get(k, []) if isinstance(d.get(k, []), list) else [d.get(k, 0)] if isinstance(d.get(k, 0), (int, float)) else 0) 
        #         for d in infos)
        #     )
        #     for k in set.union(*[set(d) for d in infos])
        # })
